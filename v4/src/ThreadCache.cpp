...
void* ThreadCache::fetchFromCentralCache(size_t index){
    ...
    // 从中心缓存批量获取内存
    void* start = CentralCache::getInstance().fetch_range(index, batchNum);
    if (!start) 
        return nullptr;
...
void ThreadCache::returnToCentralCache(void* start, size_t size)
{
    ...
    // 获取对齐后的实际块大小
    size_t alignedSize = SizeClass::round_up(size);
    ...
    if (splitNode != nullptr) 
    {
        ...
        *reinterpret_cast<void**>(splitNode) = nullptr; // 断开连接
        ...
        {
            CentralCache::getInstance().return_range(nextNode, returnNum * alignedSize, index);
        }
    }
}
